...
no_whitespace_matcher = re.compile(r"[^\s]")
...
def get_string_for_region(view, region):
    comments = overlapping_regions(region, view.find_by_selector("comment"))
    strings = overlapping_regions(region, view.find_by_selector("string"))
...
operator_split_matcher = re.compile(r"[ \[\]\(\)\{\}]")
